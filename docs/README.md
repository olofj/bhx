...
| [`os-notes.md`](os-notes.md) | Per-OS gotchas and workarounds for guest distros booted on the L2CPU — what works out of the box, what needs operator setup (e.g. AlmaLinux 10 needs a one-line `grubby` to redirect console to `hvc0`), what's blocked (Ubuntu 25.10 / 26.04 RVA23 ABI, AlmaLinux 10 dma-pool splat). Cheat-sheet for new operators. |
