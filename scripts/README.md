...
| `soak_virtio_burst.py` | Sustained multi-queue virtio burst regression test for #184: drives 16-job direct=1 fio randwrite + a tight `printf` loop to `/dev/console` for `DURATION_SEC` seconds, while sampling the daemon's `/metrics` endpoint every 1 s into a CSV. Two assertion modes via `MODE=`: `baseline` (against V1 dispatch — must reach >= 50% kick-ring fill, drops are informational), `v2` (against post-#189 V2 dispatch — must produce zero `kick.*drop`/`rescue`/`throttle.*ENGAGE` log lines and `bhx_dispatch_passes_total > 0`). Auto-strips `/etc/init.d/S99-virtio-cycle-test` from a private `/tmp/burst-rootfs.ext4` copy so getty actually fires (the standard buildroot rootfs is wired for the #156 cycle test). |
